...
class CenterIntersection(nn.Module):
    ...
    def __init__(self, dim):#接受的参数是实体的维度
        super(CenterIntersection, self).__init__()
        self.dim = dim
        self.layer1 = nn.Linear(self.dim, self.dim)
        self.layer2 = nn.Linear(self.dim, self.dim)#定义了两个线性层，输入输出的维度都是dim

        nn.init.xavier_uniform_(self.layer1.weight)
        nn.init.xavier_uniform_(self.layer2.weight)#使用xavier均匀分布初始化两个线性层的权重

    def forward(self, embeddings):
        layer1_act = F.relu(self.layer1(embeddings)) # (num_conj, dim) 嵌入进行线性变换之后，通过Relu激活函数进行输出 获得layer1_act
        attention = F.softmax(self.layer2(layer1_act), dim=0) # (num_conj, dim) layer1_act经过第二层线性变换，应用softmaxe函数 获得attention
        embedding = torch.sum(attention * embeddings, dim=0) #attention和embedding逐元素相乘，然后沿着第0维（每一列的元素相加）求和，得到embedding
...
class KGReasoning(nn.Module):
    def __init__(self, nentity, nrelation, ntype, entity2type, relation2type, hidden_dim, gamma,
                 geo, query_name_dict, test_batch_size=1,
                 use_cuda=False, proj_mode=(1600, 2), center_reg=0.2,
                 neighbor_ent_type_samples=32):
...
######################################################################
                ...
                height_entity_embedding = self.ent_neighbor_type_agg(height_entity_embedding, entity_neighbor_type_embedding)#在ConE模型中不存在高度这个参数，这里同样使用了类型聚合！！！
######################################################################
            ...
            for i in range(len(query_structure[-1])):
                ...
                # projection映射操作
                    ...
                    # height_r_embedding = self.angle_scale(
                    #     height_r_embedding, self.axis_scale
                    # )
